...
def evaluate_rag_model(args, clipmodel, processor, tokenizer, model, eval_df):
    ...
    i2t_features = h5py.File(args.i2t_features_path, 'r')
    ...
    for idx in tqdm(range(len(eval_df))):
        ...
        if args.features_path is not None:
            feature = torch.tensor(features[image_id][()])
            i2t_feature = torch.tensor(i2t_features[image_id][()]).unsqueeze(0)
            encoder_last_hidden_state = torch.cat([feature, i2t_feature], dim=0).unsqueeze(0).cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--i2t_features_path", type=str, default='i2t_features/val.hdf5', help="H5 file with cached input image features")
