...
class TrainDataset(Dataset):
    def __init__(self, df, features_path, it2_features_path, tokenizer, rag=False, template_path=None, k=None, max_caption_length=25):
        self.df = df
        self.tokenizer = tokenizer
        self.features = h5py.File(features_path, 'r')
        self.i2t_features = h5py.File(it2_features_path, 'r')
    ...
    def __getitem__(self, idx):
        ...
        features = torch.tensor(self.features[self.df['cocoid'][idx]][()])
        i2t_features = torch.tensor(self.i2t_features[self.df['cocoid'][idx]][()]).unsqueeze(0)
        encoder_outputs = torch.cat([features, i2t_features], dim=0)
