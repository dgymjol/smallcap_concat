...
class SmallCap(PreTrainedModel):
    ...
    def __init__(
        ...
        self.clip_model, self.preprocess_train, self.preprocess_val = load("ViT-L-14",jit=False)       
    ...
    def encode_text_img(self, text_inputs, img_tokens):
        ...
        x = self.clip_model.token_embedding(input_ids)
        ...
        try:
            ...
            x = self.clip_model.transformer(x.half())
        ...
        # x = (x[torch.arange(x.size(0)), collect_ind+1] @ self.clip_model.text_projection).unsqueeze(1)
...
####################################
    ...
    def forward(
        ...
        encoder_outputs=None, # (org_features + i2t_features)
        ...
        **kwargs,
        ...
        if isinstance(encoder_outputs, BaseModelOutput) or isinstance(encoder_outputs, BaseModelOutputWithPooling):
            encoder_outputs = encoder_outputs[0] #(org : 64, 51, 768)

        i2t_features = encoder_outputs[:, -1, :] # ([64, 768])
        features = encoder_outputs[:, :50, :] # ([64, 50, 768])
        
        token_features = self.img2text(i2t_features) # ([64, 768])

        with torch.no_grad():
            text_inputs = self.clip_tokenizer(["a photo of"], padding=True, return_tensors="pt")
            # text_inputs = tokenize("a photo of")
            i2t_outputs = self.encode_text_img(text_inputs, token_features)  # (64, 77, 768)
            encoder_outputs = BaseModelOutput(
                                last_hidden_state=torch.cat([features, i2t_outputs], dim=1).float(),
                                hidden_states=None,
                                attentions=None,
                            )
