...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        try:
            r = self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
        except:
            x = x.half()
            r = self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask.half())[0]
        return r
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        x = self.conv1(x.cuda())  # shape = [*, width, grid, grid]
